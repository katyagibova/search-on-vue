      ...
      <v-col cols="8">
        <v-row style="margin-top: 15px">
          <v-text-field
            ...
            solo
          ></v-text-field>
          <v-btn color="#625AD8" style="color: #fff">Найти</v-btn>
        </v-row>
        <v-row>
          <v-col cols="4">
            <v-select
        ...
        <div v-else>
          <v-row v-if="allCards == true && noCategory == false">
            <div v-for="(item, i) in PRODUCTS" :key="i">
              <div v-if="category == item.category">
                <v-card
                  elevation="1"
                  tile
                    ...
                    <v-list-item-content>
                      ...
                      <v-row>
                        <v-col cols="3">
                          <v-list-item-subtitle
                            >Размерность:
                            {{ item.dimension }}</v-list-item-subtitle
                          >
                        </v-col>
                        <v-col cols="4">
                          <v-list-item-subtitle
        ...
        <div v-if="selectedCards == true">
          <v-row v-for="(item, i) in PRODUCTS" :key="i">
            <div v-for="(type, t) in selectedType" :key="t">
              <div v-for="(prof, p) in selectedProfessionalism" :key="p">
                <div v-for="(dim, d) in selectedDimension" :key="d">
                  <!-- <div
                ...
                :key="c"> -->
                  <div
                    v-if="
                      ...
                      type == item.type &&
                      prof == item.professionalism &&
                      dim == item.dimension &&
                      item.price >= selectedFromField &&
                      item.price <= selectedToField
                    "
                  >
                    <v-card
                      elevation="1"
    ...
    <v-snackbar v-model="snackbar" rounded="pill">
      <template>
        <v-row>
          <v-spacer></v-spacer>
          <v-col cols="7">
            <v-btn color="#625AD8" @click="toFind">Отфильтровать</v-btn>
          </v-col>
          <v-spacer></v-spacer>
        </v-row>
      </template>
...
export default {
  ...
  methods: {
    ...mapActions(["GET_SKATES", "GET_PRODUCTS"]),
    toFind() {
      if (
        this.type.length == 0 &&
        this.professionalism.length == 0 &&
    ...
    newProfessionalism() {
      if (this.category.trim() == "") {
        ...
        for (let i = 0; i < this.PRODUCTS.length; i++) {
          if (this.PRODUCTS[i].category == this.category) {
            this.newProf = this.newProf.concat(
              this.PRODUCTS[i].professionalism
            );
    ...
    minMaxPrice() {
        for (let i = 0; i < this.PRODUCTS.length; i++) {
            if (this.PRODUCTS[i].category == this.category) {
                this.forMaxPrice = this.forMaxPrice.concat(this.PRODUCTS[i].price);
                this.forMinPrice = this.forMinPrice.concat(this.PRODUCTS[i].price);
            }
        }
        this.forMaxPrice = Math.max.apply(null, this.forMaxPrice)
        if(this.forMaxPrice == '-Infinity'){
            this.toField = 0
        } else{
            this.toField = this.forMaxPrice
        }
        this.forMinPrice = Math.min.apply(null, this.forMinPrice)
        if(this.forMinPrice == 'Infinity'){
            this.fromField = 0
        } else{
            this.fromField = this.forMinPrice
        }
    },
  },
  computed: {
    ...mapGetters(["SKATES", "PRODUCTS"]),
    filterType() {
